    ...
    json_build_object(
        ...
        'meta', json_build_object(
            'profile', json_build_array(
                'https://build.fhir.org/ig/arkhn/arkhn-ig-osiris/StructureDefinition/tumor-pathology-event'
            )
        ),
        ...
        json_build_array(
            CASE
                WHEN condition."extension_due_to_valuereference" IS NOT NULL
                    THEN json_build_object(
                        'url',
                        'https://build.fhir.org/ig/arkhn/arkhn-ig-osiris/StructureDefinition/dueTo-tumor-pathology-event',
            ...
            CASE
                WHEN condition."extension_laterality_valueCodeableConcept_coding_code" IS NOT NULL
                    THEN json_build_object(
                        'url', 'https://build.fhir.org/ig/arkhn/arkhn-ig-osiris/StructureDefinition-laterality.html',
            ...
            CASE
                WHEN condition."extension_histology_extension_histologygradetype" IS NOT NULL
                    AND condition."extension_histology_extension_histologygradevalue" IS NOT NULL
                    THEN json_build_object(
                        'url', 'https://build.fhir.org/ig/arkhn/arkhn-ig-osiris/StructureDefinition/histologygrade',
                        'extension', json_build_array(
                            CASE
                                WHEN condition."extension_histology_extension_histologygradetype" IS NOT NULL
                                    THEN json_build_object(
                                        'url', 'histologyGradeType',
                                        'valueString', condition."extension_histology_extension_histologygradetype"
                                    ) END,
                            CASE
                                WHEN condition."extension_histology_extension_histologygradevalue" IS NOT NULL
                                    THEN json_build_object(
                                        'url', 'histologyGradeValue',
                                        'valueString', condition."extension_histology_extension_histologygradevalue"
                                    ) END
                        )
                    ) END
        )
...
FROM
    {{ ref('osiris_master_tab_Condition_tumorPathologyEvent') }} AS condition
LEFT JOIN
