...
extension_extension_modandtech AS (
    SELECT
        ...
        json_build_object(
            ...
            'url', 'http://fhir.arkhn.com/osiris/StructureDefinition/modalityAndTechnique'
        ) AS fhir
    FROM
        unique_modand_tech
    LEFT JOIN
...
extension_extension_volume AS (
    SELECT
        ...
        json_build_object(
            ...
            'url', 'http://fhir.arkhn.com/osiris/StructureDefinition/doseToVolume'
        )
        AS fhir
    FROM
...
extension_extension_energyorisotope AS (
    SELECT
        ...
        json_build_object(
            ...
            'url', 'http://fhir.arkhn.com/osiris/StructureDefinition/energyOrIsotope'
        ) AS fhir
    FROM
...
extension_extension_basedon AS (
    SELECT
        ...
        json_build_object(
            ...
            'url', 'http://fhir.arkhn.com/osiris/StructureDefinition/basedOn'
        ) AS fhir
    FROM
        unique_basedon
),
...
extension AS (
        ...
        CASE
            WHEN
                unique_extension."extension_reasonReplanification" IS NOT NULL THEN json_build_object(
                    'url', 'http://fhir.arkhn.com/osiris/StructureDefinition/reasonReplanification',
                    'valueCodeableConcept', json_build_object(
                        'coding', json_build_array(
                            json_build_object(
    ...
    SELECT
        unique_extension."id_planrt",
        json_build_object(
            'url', 'http://fhir.arkhn.com/osiris/StructureDefinition/algorithmName',
    ...
    json_build_object(
        ...
        'meta', json_build_object(
            'profile', json_build_array(
                'http://fhir.arkhn.com/osiris/StructureDefinition/radiotherapy-plan'
            )
        ),
...
LEFT JOIN
    ...
    ON planrt."category_coding_code" = codeableconcept_category."code"
LEFT JOIN
    {{ ref('osiris_master_tab_Procedure_radiotherapy_codeableconcept') }}
