...
study_series AS (
    SELECT
        ...
        "series"."id_series",
        COALESCE("location"."identifier_value", 'Unknown') AS "identifier_value",
        CASE
...
study_series_commonimage_modality AS (
    SELECT
        ...
        COALESCE("study_series_commonimage"."endpoint", 'Unknown') AS "endpoint",
        ...
        COALESCE("study_series_commonimage"."description", 'Unknown') AS "description",
        "study_series_commonimage"."series_performer_actor",
        "study_series_commonimage"."series_number",
        COALESCE("study_series_commonimage"."series_uid", 'Unknown') AS "series_uid",
        CASE
            WHEN "study_series_commonimage"."id_imagingstudy" IS NULL AND dximage."id_dximage" IS NOT NULL
                THEN 'DX'
            WHEN "study_series_commonimage"."id_imagingstudy" IS NULL AND mrimage."id_mrimage" IS NOT NULL
            ...
            WHEN "study_series_commonimage"."series_modality_code" IS NOT NULL
                THEN "study_series_commonimage"."series_modality_code"
        END AS "series_modality_code",
        COALESCE("study_series_commonimage"."series_started", '1000-01-01') AS "series_started",
        COALESCE("study_series_commonimage"."series_description", 'Unknown') AS "series_description",
        ...
        END AS "series_extension_patient_height",
        COALESCE("study_series_commonimage"."series_extension_slice_thickness", 'Unknown')
        AS "series_extension_slice_thickness",
        COALESCE("study_series_commonimage"."series_extension_pixel_spacing",
            'Unknown') AS "series_extension_pixel_spacing",
        ...
        CASE
            ...
            WHEN study_series_commonimage."series_modality_code" = 'NM'
                THEN 'Nuclear Medicine Image Storage'
            WHEN "study_series_commonimage"."id_imagingstudy" IS NULL AND nmimage."id_nmimage" IS NOT NULL
                THEN 'Positron Emission Tomography Image Storage'
            WHEN study_series_commonimage."series_modality_code" = 'RTSTRUCT'
